...
class AdlrAutoresume:
    ...
    Usage:
        ...
        for epoch in range(start_epoch, config.train.num_epochs + 1):
            # training loop
            ...
            # check for autoresume after each epoch
            should_stop = autoresume.check_autoresume(epoch, model, config)
            if should_stop:
                break
        ```
    """
    ...
    def check_autoresume(self, epoch, model, config) -> bool:
        if self.terminate and not self._autoresume_sent:
            self._request_autoresume(epoch, model, config)
...
def train(config, device):
    ...
    for dataset_cfg in config.train.data:
        ...
        if config.train.load_env_meta:
            ...
            env_meta = {"env_name": "dummy"}
            assert not config.experiment.rollout.enabled, "cannot perform rollout without env_meta"
    ...
    for epoch in range(1, config.train.num_epochs + 1): # epoch numbers start at 1
        ...
        if config.experiment.save.enabled:
            ...
            autoresume_check = False
            if autoresume is not None:
                autoresume_check = autoresume.check_autoresume(epoch, model, config)
            should_save_ckpt = (time_check or epoch_check or epoch_list_check or autoresume_check)
        ckpt_reason = None
        if should_save_ckpt:
            last_ckpt_time = time.time()
            ckpt_reason = "time"
        ...
        if autoresume_check:
            print("Autoresume checkpointing at epoch {}".format(epoch))
            break
