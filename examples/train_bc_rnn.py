...
def articulate_hyperparameters(config):
    """
    Sets robosuite-specific hyperparameters.
    ...
    config.observation.modalities.obs.rgb = ["hand_camera"]                # no image observations
    config.observation.modalities.goal.low_dim = []             # no low-dim goals
    config.observation.modalities.goal.rgb = []               # no image goals
...
DATASET_TYPES = {
    ...
    "articulate": {
        "default_dataset_func": TestUtils.example_articulate_dataset_path,
        "hp": articulate_hyperparameters,
    },
    "momart": {
